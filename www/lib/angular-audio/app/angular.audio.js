...
.directive('ngAudio', ['$compile', '$q', 'ngAudio', function($compile, $q, ngAudio) {
    return {
        ...
        controller: function($scope, $attrs, $element, $timeout) {
            ...
            var myOperation = function(){
                if(!$scope.playing)
                {
                        $element.children(".blay-ztop").removeClass("ion-ios-musical-notes").addClass("ion-android-hand");
                        //$element.replaceText( /\bplay\b/gi, 'Stop' );
                    ...
                    $element.children(".blay-ztop").removeClass("ion-android-hand").addClass("ion-ios-musical-notes");
                    //$element.text($element.text().replace("Stop","Play"));
                       $timeout(function() {
                            audio.restart();
                        }, 5);
